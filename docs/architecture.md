...
团队现在可以创建多个项目。项目是实验计划的管理容器而不是计划网络节点；工作区、正文、事件和附件读写均显式限定到当前项目。项目列表负责选择上下文，右侧详情栏负责元数据、成员、资源、生命周期和审计活动管理。

项目权限分为负责人、成员和访客。团队管理员可查看全部项目并执行管理操作，但只有加入项目或开启有审计记录的限时应急编辑后，才能修改科研内容。归档项目默认从列表隐藏并整体只读。旧版团队唯一项目会迁移为“历史实验项目”。完整决策与边界见 `docs/decisions/0004-project-containers-and-access.md`，交互与验收规则见 `docs/project-management-spec.md`。

## 12. 科研问题与个人看板（2026-08-20）

科研问题以团队 PostgreSQL 为事实源，可关联同一项目中的实验节点，并记录答案结论与回填说明。问题及其关联关系随项目权限边界读写，不依赖浏览器本地草稿。

`/my-board` 是当前用户的跨项目工作入口。服务端只查询用户实际加入的项目，再汇总“紧急、进行中、等待、持续关注”四类未完成实验节点；已完成项目和无权访问的项目不会出现在看板中。节点卡片包含项目、计划路径、领域、标签、计划完成日期和未完成前置依赖数量，并支持深链接回到对应项目与节点。

看板上的状态更新使用独立的版本化 API：事务内锁定项目、同时比较项目版本与节点版本、更新状态并写入审计事件。冲突返回 `409 VERSION_CONFLICT`，客户端重新读取看板，避免覆盖其他成员的更新。筛选条件仅作为个人浏览器偏好保存在 `localStorage`，不进入团队数据或审计记录。
