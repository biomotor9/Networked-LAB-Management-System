...
export async function POST(request: Request, context: Context) {
  ...
  const now = new Date();
  const [updated] = await db.update(projects).set({ archivedAt: now, archivedBy: actor.id, updatedAt: now, version: Number(payload.version) + 1 })
  ...
  await recordAudit({ action: "project.archived", targetType: "project", targetId: projectId, projectId, actorUserId: actor.id, teamId: actor.teamId });
