...
export async function POST(request: Request, context: Context) {
  ...
  const payload = await request.json() as { version?: unknown; confirmIncomplete?: unknown };
  ...
  await recordAudit({ action: "project.completed", targetType: "project", targetId: projectId, projectId, actorUserId: actor.id, teamId: actor.teamId, metadata: { incompletePlans: incomplete.length } });
